...
extension API {
	private func _login(email: String, password: String) async throws -> LoginResponse {
		...
		let parameters: [String: String] = ["Content-Type": "application/json;charset=utf-8"]
		let body = LoginRequestBody(email: email, password: password)
		
		URLSession.shared.configuration.httpCookieStorage?.removeCookies(since: Date.distantPast)
		
		return try await request(.post, url: url, parameters: parameters, body: body, authorization: .none)
	}
